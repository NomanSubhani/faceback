...
<!--Head fb text-->
<div style="position:fixed;left:4.05%;top:0.8%;font-size:25;font-weight:900; z-index:2;"> <a href="Home.php" style="color:#FFFFFF; text-decoration:none;" onMouseOver="on_head_fb_text()" onMouseOut="out_head_fb_text()"> <font face="myFbFont"> faceback </font> </a> </div>
...
error_reporting(1);
		...
		$query1=$connect->query("select * from users where Email='$user'");
		$rec1=$query1->fetch_array();
		$userid=$rec1[0];
		$query2=$connect->query("select * from user_profile_pic where user_id=$userid");
		$rec2=$query2->fetch_array();
...
<?php
	 $query_online=mysqli_query($connect,"select * from user_status where status='Online'");
...
<?php			
	 ...
	 while($online_data=mysqli_fetch_array($query_online))
	 {
		...
		$query_online_user_data=mysqli_query($connect,"select * from users where user_id=$online_user_id");
		$query_online_user_pic=mysqli_query($connect,"select * from user_profile_pic where user_id=$online_user_id");
