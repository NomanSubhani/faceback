<?php
	...
	if(isset($_SESSION['tempfbuser']))
	{
		...
		$que1=$connect->query("select * from users where Email='$user' ");
		$rec=$que1->fetch_array();
		$userid=$rec[0];
		$que2=$connect->query("select * from user_secret_quotes where user_id=$userid");
		$rec2=$que2->fetch_array();
		$q2=$rec2[3];
		$a2=$rec2[4];
		if($q2=="" && $a2=="")
		{
			$que3=$connect->query("select * from user_secret_quotes where user_id=$userid");
			$count3=$que3->num_rows;
			if($count3>0)
			{
...
<?php
	if(isset($_POST['Finish']))
	{
		...
		$connect->query("update user_secret_quotes set Question2='$que2',Answer2='$ans2' where user_id=$userid");
		
		$que_user_data=$connect->query("select * from users where Email='$user';");
		$user_data=$que_user_data->fetch_array();
		$userid=$user_data[0];
		$user_join_time=$user_data[6];
		$connect->query("insert into user_post(user_id,post_txt,post_time,priority) values($userid,'Join Faceback','$user_join_time','Public');");
		$connect->query("insert into user_status values($userid,'Online')");
		$connect->query("insert into user_info(user_id) values($userid)");
