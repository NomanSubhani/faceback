<?php
	...
	if(isset($_SESSION['fbadmin']))
	{
		...
		if(isset($_POST['notice_users']))
		{
			...
			$que_users=mysqli_query($con,"select * from users");
			while($users_data=mysqli_fetch_array($que_users))
			{
				$user_id=$users_data[0];
				mysqli_query($con,"insert into users_notice(user_id,notice_txt,notice_time) values($user_id,'$notice_txt','$notice_time')");
			}
		}
