...
<?php
	if(isset($_POST['mdelete']))
	{
		...
		mysqli_query($con,"delete from users where user_id=$dm_id");
	}
	if(isset($_POST['fdelete']))
	{
		$df_id=intval($_POST['user_female_id']);
		mysqli_query($con,"delete from users where user_id=$df_id");
	}
?>
<?php
...
<body>
<?php
	$all_users_que=mysqli_query($con,"select * from users");
...
<?php
	$all_male_que=mysqli_query($con,"select * from users where Gender='Male' order by user_id desc");
...
<?php
	$all_female_que=mysqli_query($con,"select * from users where Gender='Female' order by user_id desc");
